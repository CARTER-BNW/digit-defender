...
- `ui/hud.py` — balance, targets, toolbar (TOOLS/HOTKEYS), hover/selection info line, messages; panels/text follow the text size (`Hud.px`/`Hud.text`), the toolbar the button size; portrait layout when h > w (panels stack under the right column via `_draw_rows`, toolbar wraps to 2x7, minimap above the toolbar and the Android rows via `overlay_top`); fold-away minimap (`minimap_hidden`: click = fold into a Map button, right click = look there; config "minimap")
...
- `android/mobile/entry.py` — the APK's main: MobileGame (Game + touch layer, drops SDL's mirrored mouse events, saves on APP_WILLENTERBACKGROUND, F11 no-op, `refit()` on rotation) and MobileMenu (soft keyboard on New World, Settings without the fullscreen row), pygame patches (Back key -> Escape in every event loop, key.get_mods, mouse.get_pos), SCALED canvas 720 px along the short side (`logical_size`, `fit_display` re-creates it when the window's aspect flips; SDL orientation hint = follow the phone), saves redirected to the app's private dir; `--desktop` emulates the phone in a window (mouse = finger)
